...
function computeSidebarFlowLayeredLayout(stepIds = [], edges = [], start = '') {
    ...
    // DFS 剔除回环边（指向仍在递归栈上的节点），剩余前向边构成 DAG
    ...
    const startId = idSet.has(start) ? start : ids[0];
    dfs(startId);
    ids.forEach((id) => {
        if (!visited.has(id)) {
            dfs(id);
    ...
    // Kahn 拓扑序上取最长路径作为列深度，保证节点总在其所有前驱之后
    const depth = new Map();
    ...
    // 逐列分配行号：按前驱行号的重心排序，true 略偏上、false 偏下；冲突时向下顺延
    ...
    let minRow = Infinity;
    ids.forEach((id) => {
        minRow = Math.min(minRow, row.get(id) || 0);
    });
    ids.forEach((id) => {
        positions.set(id, {
...
function renderSidebarFlowCanvas(cardData = null) {
    ...
    const edgeGroupCounters = new Map();
    const takeEdgeGroupIndex = (key) => {
        const next = edgeGroupCounters.get(key) || 0;
        edgeGroupCounters.set(key, next + 1);
        return next;
    };
    const edgeMarkup = sidebarFlowState.edges.map((edge) => {
        ...
        // 同一出口/入口的多条线依次错开，避免重叠缠绕
        const sourceGroupIndex = takeEdgeGroupIndex(`from:${edge.from}|${fromPort}|${sy}`);
        const targetGroupIndex = takeEdgeGroupIndex(`to:${edge.to}|${toPort}`);
        const spread = Math.min(Math.max(sourceGroupIndex, targetGroupIndex), 3) * 12;
        const geometry = buildSidebarFlowEdgeGeometry(sx, sy, tx, ty, fromPort, toPort, spread);
        const path = geometry.path;
        const label = String(edge.label || 'next').trim() || 'next';
        const labelMarkup = sourceIsCondition && (edgeLabel === 'true' || edgeLabel === 'false')
            ? `<text class="sidebar-flow-edge-label" x="${geometry.labelX}" y="${geometry.labelY}">${escapeHtml(label)}</text>`
            : '';
        return `
