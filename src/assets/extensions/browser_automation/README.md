...
一个基于 Chrome Manifest V3 的浏览器扩展，用于浏览器任务的自动化执行，并可自动接入本机 **AI-FREE** 的「AI 控制」。插件无需单独登录；多个浏览器插件连接会在软件中分开显示和选择。
...
3. 在模型下拉框左侧的「浏览器」下拉框中选择要控制的插件连接。
4. 多个浏览器会作为不同连接分别显示；每段对话只把工具调用派发给当前选中的浏览器。
