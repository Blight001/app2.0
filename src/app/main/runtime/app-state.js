...
function createAppState({ setRemoveWatermarkEnabled = () => {} } = {}) {
  ...
  const state = {
    ...
    activeTabId: null,
    registrationWebTabId: null,
    registrationWebPageOpening: false,
    sideView: null,
    consoleWindow: null,
...
// 设置/更新/持久化：applyPluginSettings的具体业务逻辑。
  ...
  return {
    ...
    getRegistrationWebTabId: () => state.registrationWebTabId,
    setRegistrationWebTabId: (next) => { state.registrationWebTabId = next; },
    getRegistrationWebPageOpening: () => state.registrationWebPageOpening,
    setRegistrationWebPageOpening: (next) => { state.registrationWebPageOpening = next; },
