(function initBrowserSettingsFormModule() {
  ...
  function fillBrowserSettingsForm(deps, settings, runtimeInfo = {}) {
    ...
    const exitIp = section(current, 'exitIp');
    deps.setValue('browser-locale', language.value);
    deps.setValue('browser-timezone', timezone.value);
    deps.setValue('exit-ip', exitIp.ip);
    deps.setValue('exit-ip-region', exitIp.region);
    deps.setValue('exit-ip-country', exitIp.country);
    deps.setValue('exit-ip-city', exitIp.city);
    deps.setValue('geo-longitude', geolocation.longitude);
    deps.setValue('geo-latitude', geolocation.latitude);
    deps.setValue('geo-accuracy', geolocation.accuracy);
