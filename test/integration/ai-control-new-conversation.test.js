...
test('最近聊天下方最多展示三条浏览器快速启动记录并可点击打开', async () => {
  const messages = new FakeElement('div');
  const welcome = new FakeElement('div');
  welcome.className = 'ai-chat-welcome';
...
test('新对话欢迎区不展示控制目标说明', () => {
  ...
  vm.runInContext("state.currentBrowserIds = ['browser-1']; state.currentCardId = 'card-1'", context);

  vm.runInContext('renderWelcome()', context);

  const welcome = messages.children[0];
  assert.match(welcome.innerHTML, /有什么可以帮你？/);
  assert.doesNotMatch(welcome.innerHTML, /已连接|当前卡片|AI 将在所选浏览器中执行操作/);
});
