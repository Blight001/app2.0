...
test('卡片写入校验接受合法分支流并拒绝悬空节点与未知步骤类型', () => {
  ...
  assert.throws(() => context.validateCardDataForWrite({
    website: 'https://example.com', steps: [{ type: 'external_script', script: 'alert(1)' }],
  }), /卡片校验失败/);
  assert.throws(() => context.validateCardDataForWrite({
    website: 'https://example.com', steps: [{ type: 'condition', condition_mode: 'js', expression: 'true' }],
  }), /不允许使用 JS 条件/);
});

test('MCP 运行拒绝历史卡片中的任意页面脚本', () => {
  const context = createContext();
  assert.throws(() => context.assertMcpSafeCardData({
    steps: [{ type: 'external_script', script: 'document.body.remove()' }],
  }), /MCP 禁止/);
  assert.doesNotThrow(() => context.assertMcpSafeCardData({
