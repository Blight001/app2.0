...
test('AI MCP prompt 以紧凑文本注入多浏览器路由和页面操作策略', () => {
  ...
  assert.match(prompt, /传 change_browser/);
  assert.match(prompt, /页面变化后刷新 ref/);
  assert.match(prompt, /真实 downloadUrl/);
  assert.match(prompt, /未成功不得声称完成/);
  assert.ok(prompt.length < 800, `MCP 前置提示应保持紧凑，实际 ${prompt.length} 字符`);
...
test('没有浏览器连接时 prompt 不会诱导 AI 虚构浏览器 MCP', () => {
  ...
  assert.doesNotMatch(context.modelMessages[0].content, /用 browser_tab 管理网页标签/);
});
