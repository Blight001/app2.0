...
嵌入成功后会再次验证 `GetParent(browserHwnd) == hostHwnd` 和 `IsChild(hostHwnd, browserHwnd)`；验证失败则阻止状态进入 `ready`。运行态包含 `embedded: true` 与 `productName: "AI-FREE 浏览器"`。Native Host 会清除外部浏览器的 `WS_EX_APPWINDOW`，把 Browser HWND 作为子窗口显示在 Electron 预留区域，并同步宿主尺寸。

## 品牌边界

- 客户端运行时标签、宿主窗口标题和嵌入子窗口标题统一显示为“AI-FREE 浏览器”。
...
完整锁定清单位于 `chromium-fork/version-lock.json`，Visual Studio
组件清单位于 `chromium-fork/toolchain.vsconfig`，GN 参数位于
`chromium-fork/args.gn`。本构建使用 `symbol_level=0`，不生成大体积 PDB。
...
补丁按 `chromium-fork/patches/series` 固定顺序应用：

1. `0001-ai-free-branding.patch`：公司、产品、安装器和版本资源品牌。
2. `0001a-ai-free-ui-strings.patch`：产品名、窗口标题、任务管理器、设置/版本页
   使用的 AI-FREE UI 字符串。
3. `0002-ai-free-runtime-switches.patch`：五个 `--hs-*` 私有参数。
4. `0003-ai-free-runtime-pipe.patch`：长度前缀 JSON Named Pipe 握手、heartbeat、
   六条双向命令及 Cookie/Storage 会话导入。
5. `0004-ai-free-browser-window-handshake.patch`：Browser 顶层 HWND 主动上报。
6. `0005-ai-free-windows-icon.patch`：从 `chromium-fork/assets/AI-FREE.png`
   生成的 16/32/48/64/128/256 多尺寸 Windows 产品图标（二进制 Git patch）。
7. `0006-ai-free-embedded-input-activation.patch`：仅在显式
...
均为 `AI-FREE Browser 150.0.7871.114`。
...
- 独立真实输入验收：通过 `focusChildWindow` 聚焦嵌入 Browser HWND，发送
  ...
  持续更新，窗口标题为 `Example Domain - AI-FREE Browser`。
- 最终 EXE 关联图标已提取并与 `AI-FREE.png` 目视核对一致；产品/文件描述为
  `AI-FREE Browser`，版本为 `150.0.7871.114`。
...
5. 所有步骤成功后才发送 `navigate(targetUrl)`；任一步失败都会拒绝业务调用并关闭
   新建标签。
