...
- `src/app/main/browser-runtime/`：统一运行时接口、Chromium 状态机、启动器、命名管道协议、心跳与故障状态。
...
- 标签管理器只支持内置 `chromium` 模式。Phase 2 起 `npm start` 进入正式 Fork 模式；`npm run start:prototype` 仅用于系统浏览器原型联调。
- 正式 Chromium 模式启动失败会直接返回明确错误，不搜索系统 Chrome/Edge 或外部路径。只有显式设置
...
网页运行时已统一为项目内置 Chromium，旧兼容路径已经删除。
...
10. `0007-ai-free-embedded-window-lockdown.patch`：仅在显式
    `--hs-embed-mode=child-window` 下禁用宿主子窗口的关闭、最小化、最大化和尺寸调整；
    地址栏、新建窗口、新建/关闭标签页、标签拖动等 Chromium 原生入口保持可用。
...
12. `0009-ai-free-embedded-omnibox-read-only.patch` 与
    `0010-ai-free-embedded-toolbar-simplification.patch` 已从补丁队列停用：嵌入模式恢复
    完整地址栏，可聚焦、输入、粘贴、拖放网址并提交导航，工具栏内部控件也恢复显示。
13. `0011-ai-free-embedded-extension-actions-pinned.patch`：仅在嵌入模式下将所有
    已加载且带工具栏 Action 的插件默认固定到工具栏；该补丁只修改
    `ToolbarActionsModel`，不隐藏或禁用地址栏。

嵌入模式保留 Chromium 原生窗口与标签页用户入口；扩展也可继续通过
`chrome.tabs` API 新建空白页：
...
- `npm run check:chromium-embedded-policy`：通过；嵌入宿主窗口样式保护、地址栏与
  窗口/标签用户入口恢复、插件工具栏默认固定、扩展 `chrome.tabs.create` 和扩展
  浮窗失焦关闭策略检查通过。
- Chromium 增量构建：地址栏、工具栏、窗口命令与标签栏涉及的 11 个 C++ 文件
  均成功编译，`chrome.dll` 与主程序完成链接并已重新 stage 到
  `resources/chromium/`；正式 Fork
  双 Profile 嵌入短验收通过。
