...
本文档记录当前软件实际向 AI 提供的 MCP 工具。工具分为浏览器窗口管理、绑定软件视觉/UI Automation，以及内置浏览器自动化三组。`software_ui` 仅在当前活动栏目是已停靠外部软件时按需出现，避免为普通对话增加 prompt 成本。
...
| 软件视觉与 UI Automation | 1 | 当前活动栏目绑定了可用外部软件窗口 | [`ai-software-ui-tools.js`](../src/app/main/services/ai-software-ui-tools.js) |
...
| `software_ui` | `aifree.software_ui` | 通过窗口截图、UI Automation 和系统输入观察并操作当前绑定软件 |
...
- `action=observe` 默认使用 `mode=auto`：先读取精简 UIA 控件；没有可操作控件时自动通过 Windows Graphics Capture 截取绑定窗口。也可指定 `accessibility` 或 `visual`。
- 每次观察生成唯一 `observation_id`。截图坐标、UIA 点击缓存和元素引用只属于该次观察；执行动作后立即失效，并默认返回新的观察状态。
- UIA 结果包含控件 `ref`、名称、类型、当前值、屏幕区域和可用操作。默认最多 30 项，硬上限 80 项；深度默认 6、上限 10，文本和值单项最多 160 字符。
- 密码控件只返回 `password: true`，不会返回当前值。
- 操作包括 `click`/`invoke`/`mouse_click`/`double_click`/`right_click`、`type`/`set_value`、`press_key`、`scroll`、`drag`、`toggle`、`select`、`expand`、`collapse` 和 `focus`。
- `click`、`mouse_click`、`double_click`、`right_click` 直接使用 Windows `SendInput`；只有显式 `invoke` 才执行可能阻塞的 UIA 语义动作。
- 软件 UIA、Windows Graphics Capture 和输入操作均在独立 worker 中执行；第三方 UIA Provider 或图形驱动超时不会阻塞 Electron 主线程。
- 视觉点击使用截图内坐标并要求当次 `observation_id`；运行时将缩放后的图片坐标映射回物理屏幕坐标并校验 DPI、HWND 和 PID。
- 鼠标输入前会校验坐标实际命中的窗口仍属于绑定 PID；若被其他程序遮挡则拒绝点击，防止误操作。
- 软件出现模态弹窗时，观察和后续操作自动切换到该弹窗，并只将它提升到所属窗口前方；返回结果以 `popup` 标记当前根节点。
- 控件树变化后旧 `ref` 可能失效，必须重新 `observe`。UI Automation 连接和事务分别设置 2 秒、5 秒超时，避免无响应软件长期阻塞 AI。

工具不接受调用方传入 HWND/PID，也不提供全桌面任意点击。所有视觉和 UIA 操作都固定在请求开始时绑定的软件窗口及其 owned popup；截图使用 Windows Graphics Capture，失败时才使用 `PrintWindow` 兼容后备。
...
- 截图作为临时 `image_url` 消息发送，Base64 不写入工具文本、历史消息或工具活动记录；默认最长边限制为 1600×1000，并保留坐标映射。
