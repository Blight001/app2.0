...
**状态**：`0014` 权限策略、`0015` 文件选择、`0020` 页面自动化原生通道、`0021` 可见原生指针、`0022` 下载链接观察、`0023` 原生键盘/滚轮、`0024` 原生 Observe 描边、`0025` 标签管理/站点数据清理、`0026` 卡片定位及等待语义、`0027` 精确截图与键盘修饰键、`0028` 统一 ANI 动画鼠标、`0029` 点击目标及失败语义、`0030` iframe 元素坐标换算已实现；`0016`–`0019` 待实施
...
`errorCode`、失败步骤和错误消息，不再把未命中动作记录为成功。

`0030-ai-free-frame-element-coordinates.patch` 将同源 iframe 内元素的局部矩形逐层
换算到顶层页面视口，包含 iframe 边框和 CSS 缩放；点击指针、原生鼠标事件与
Observe 返回坐标由此使用同一顶层坐标系。
...
`0024-ai-free-native-observe-highlights.patch` 将 Observe 元素描边放在 Chromium
Views 层：覆盖层不进入页面 DOM、不参与命中测试，并用 `e1/e2/...` 标签对应返回
元素。交互元素为绿色、媒体为橙色、文本为蓝色，最多绘制 120 个；新 Observe
替换旧标记，导航、滚动、窗口隐藏或 0.5–30 秒超时会自动清除。
