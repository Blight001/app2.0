...
- 不重写已经通过单元测试的软件窗口捕获、输入和 Cursor Sidecar。
- 不为了目录整齐批量改名或格式化整个项目。
...
浏览器和软件窗口共享无业务状态的 `WorkspaceFrame`：

```text
┌──────────────── 顶部工作栏 ────────────────┐
│ 工作域名称 / 当前目标 / 状态 / 设置 / 窗口操作 │
├───────────────────────┬────────────────────┤
│                       │ AI                  │
│ 主内容区               ├────────────────────┤
│                       │ 自动化 / 日志        │
...
- 外部软件嵌入、Cursor Sidecar、软件设置 UI 和关闭恢复验收通过。
