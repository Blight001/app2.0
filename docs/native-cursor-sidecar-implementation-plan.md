...
- [x] 阶段 A：Win32 Host、DirectComposition、命中穿透、鼠标钩子、租约、
  当前用户 Named Pipe、心跳和独立恢复 watchdog。
- [x] 阶段 B：多栏目状态、独立坐标、激活/移除/区域同步、稳定快照、
  最小化/隐藏挂起和一次受限重启。
- [x] 阶段 C：单调时钟动画、`sequenceId` 取消、`ARRIVED`/反馈事件、
  ANI 元数据与帧预解码、静态回退、设备丢失重建和低频帧统计。
- [x] 阶段 D：Chromium 权威物理坐标预解析、同一解析结果提交、单 Profile
  事务，以及真实 Chromium Fork 构建与可信点击验收。
- [x] 阶段 E：浏览器与软件统一调用 Sidecar；Chromium 自绘鼠标源文件、
  旧资源路径命名和其他显示实现已移除。

自动化验收覆盖真实 ANI 渲染、多栏目快照、反馈和 1000 次顺序动作；60/120/144
Hz 的帧统计逻辑由原生测试覆盖。实际高刷拖尾与小于一帧延迟仍必须在具备
120/144 Hz 输出的发行测试机上人工复验，本机当前输出为 75 Hz。
